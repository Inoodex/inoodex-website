    ...
    <div class="container-fluid py-4">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200 bg-blue-600 text-white">
                <h1 class="h5 mb-0">Edit Theme</h1>
            </div>
            <div class="p-6">
                @if ($errors->any())
                ...
                <form action="{{ route('admin.themes.update', $theme->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="title" id="title"
                            ...
                            value="{{ old('title', $theme->title) }}" required>
                        @error('title')
                    ...
                    {{-- <div class="mb-3">
                        ...
                        <input type="text" name="category" id="category"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('category') is-invalid @enderror"
                            value="{{ old('category', $theme->category) }}">
                        @error('category')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div> --}}
                    ...
                    <div class="mb-3">
                        ...
                        <select name="category_id" id="category_id"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('category_id') is-invalid @enderror" required>
                    ...
                    <div class="mb-3">
                        <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Price <span class="text-danger">*</span></label>
                        <input type="number" name="price" id="price" step="0.01"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('price') is-invalid @enderror"
                            value="{{ old('price', $theme->price) }}" required>
                        @error('price')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" name="demo_url" id="demo_url"
                            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('demo_url') is-invalid @enderror"
                            value="{{ old('demo_url', $theme->demo_url) }}">
                        @error('demo_url')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('description') is-invalid @enderror"
                            rows="4">{{ old('description', $theme->description) }}</textarea>
                        @error('description')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    @if ($theme->image)
                        <div class="mb-3">
                            ...
                            <img src="{{ asset($theme->image) }}" alt="{{ $theme->title }}" class="max-w-full h-auto mb-2"
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('status') is-invalid @enderror">
                            <option value="active" {{ old('status', $theme->status) == 'active' ? 'selected' : '' }}>Active
                            </option>
                            <option value="inactive" {{ old('status', $theme->status) == 'inactive' ? 'selected' : '' }}>
                                Inactive</option>
                        </select>
                        @error('status')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                    ...
                    <a href="{{ route('admin.themes.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Cancel</a>
                </form>
            </div>
        </div>
    </div>
