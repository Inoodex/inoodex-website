    ...
    <div class="container-fluid py-4">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200">
            <div class="px-6 py-4 border-b border-gray-200 bg-blue-600 text-white">
                <h1 class="h5 mb-0">Edit Portfolio Item</h1>
            </div>
            <div class="p-6">
                @if ($errors->any())
                ...
                <form action="{{ route('admin.portfolios.update', $portfolio) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        @if ($portfolio->image)
                            <img src="{{ asset('storage/' . $portfolio->image) }}" width="150" alt="Portfolio Image">
                        @else
                    ...
                    <div class="mb-3">
                        ...
                        <select name="status" id="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none @error('status') is-invalid @enderror">
                            ...
                            <option value="inactive" {{ $portfolio->status == 'inactive' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                        @error('status')
                            <div class="mt-1 text-sm text-red-600">{{ $message }}</div>
                    ...
                    <a href="{{ route('admin.portfolios.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600">Cancel</a>
                </form>
            </div>
        </div>
    </div>
